...
R --no-save --quiet <<__R__
  ...
  read_m6a = function(file, my_tag = "", min_ml = 200, nrows=Inf, ref=FALSE, ecfilt=3.9){
  ...
  dists_df = read_m6a("$tmpd/$samplenm.$ftype", ecfilt=ecf) %>%
    group_by(fiber) %>%
    arrange(start) %>%
  ...
  if(length(unique(dists_df[["fiber"]])) < 50) {
    ...
    cat("Asymptote(Curve)=NaN\n", file=stats_file, append=TRUE)
    cat("Area(Curve)=NaN\n", file=stats_file, append=TRUE)
    cat("Count(Peaks)=NaN\n", file=stats_file, append=TRUE)
    cat("Area(Peaks)=NaN\n", file=stats_file, append=TRUE)
    cat("Noise(Curve)=NaN\n", file=stats_file, append=TRUE)
    cat("Area(Curve)/Noise(Curve)=NaN\n", file=stats_file, append=TRUE)
    cat("\n", file=stats_file, append=TRUE)
  } else {
    LAG = 2000
    acf_list = dists_df %>%
